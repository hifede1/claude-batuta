...
| **cadenas-que-github-interpreta** | Por qué el cierre de issues solo funciona en inglés y por qué los marcadores de omisión de CI se activan **al ser citados**: tres incidentes medidos, dos con control. Contiene su propia restricción — no transcribe las cadenas, las describe | 2026-08-09 | `github`, `issue`, `PR`, `pull request`, `merge`, `cerrar issue`, `omision de CI`, `checks`, `commit`, `mensaje de commit`, `idioma`, `automerge`, `workflow` | S22 | 🟢 | `docs/references/cadenas-que-github-interpreta.md` |
| **cables-bash-defensivo** | Cómo se escribe un cable de coherencia que no dé falso verde: las cinco reglas duras (sin `set -e`, `exit 2` frena, freno por intérprete, `LC_ALL=C`, solo jq/sed/grep), anclar en el literal, las tres clases de escenario sembrado, y los seis defectos propios que cada cable de este repo trajo | 2026-08-09 | `cable`, `chequeo`, `sembrar`, `bateria`, `siembra`, `escenario sembrado`, `set -e`, `exit 2`, `frenar`, `bash`, `BASH_VERSINFO`, `LC_ALL`, `falso verde`, `CI`, `script`, `shell`, `sed`, `grep`, `jq` | S22 | 🟢 | `docs/references/cables-bash-defensivo.md` |
