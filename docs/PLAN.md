...
✅ **Criterios de aceptación.** — **CERRADA 2026-08-09 (encargo `#127`), 8 de 8.**
- [x] `docs/references/cables-bash-defensivo.md` existe, con `triggers`, fecha y citas a las fuentes *(verificación: inspección + el chequeo 2 de `frontera.sh` en verde con la fila nueva en el catálogo)* → **cumplido**: generada **ANTES** del cable, con 5 reglas duras, la tabla de los seis defectos propios y su checklist. `frontera.sh` chequeo 2 ✓ **7 referencias, ninguna en falso**. **Y el orden no fue ritual: su propio checklist cazó los tres defectos silenciosos de esta sesión.**
- [x] El chequeo 9 **cuenta los `.sh` de `.github/scripts/`** y exige que cada uno esté declarado en `TESTS.gates` *(verificación: escenario sembrado — borrar un gate de la vista da ROJO nombrando cuál)* → **cumplido**: `s_trk_gate_faltante` ✓ ROJO con `(a) frontera.sh`. **Y hubo que arreglar la caja primero:** copiaba solo el cable, así que con un único `.sh` en el directorio este sub-chequeo **nacía INERTE** — pasaba sin ejercitar nada. Sembrado en falso, cazado antes de mergear.
- [x] El chequeo 9 exige **entrada de `CHANGELOG` con la fecha de `LAST_AUDIT`** *(verificación: escenario sembrado — avanzar `LAST_AUDIT` sin entrada da ROJO)* → **cumplido**: `s_trk_sin_asiento` ✓ ROJO con `(b) CHANGELOG`. **Destapó dos defectos más:** `sync_trk` dejaba el tracker a medio sincronizar (tres anti-falso-positivo se pusieron rojos **con razón**), y el sub-chequeo **anclaba donde APARECE la fecha y no donde VIVE** — se comía una fecha del texto de la entrada y devolvía ROJO donde debía FRENAR. Los dos corregidos.
- [x] El chequeo 9 exige que **`const PLAN` y el campo `plan`** cubran toda sesión CERRADA de este archivo *(verificación: escenario sembrado — cerrar una sesión sin agregarla da ROJO)* → **cumplido**: `s_trk_sesion_sin_vista` ✓ ROJO con `(c) S98`, en **las dos** vistas. **⚠️ CRITERIO REFORMULADO, con su motivo a la vista:** el marcador «CERRADA» **no existe de forma uniforme** — de 21 sesiones solo **5** lo llevan (S10, S14, S19–S21), S16–S18 dicen «N de N cumplidos», y **S01–S09, S11, S12 y S13 no dicen nada**: su estado vive solo en la tabla resumen, que **se declara a sí misma vista derivada**. Anclar ahí habría exigido **inventar el marcador y estamparlo en 16 sesiones**, o sea tocar el plano firmado para que el cable pudiera correr — el error exacto por el que `frontera.sh` retiró su tercer chequeo. Se ancla en `^## S<NN>`: el plano **sí** es fuente de qué sesiones existen; solo no lo es de en qué estado están.
- [x] Los tres sub-chequeos se **vieron FALLAR contra el repo real** antes de corregir nada *(verificación: la evidencia queda en `docs/audits/s22-*.md` con la salida cruda)* → **cumplido de la forma más fuerte disponible**: no con un escenario inventado sino **corriendo el cable nuevo contra `f1dfd85`**, el commit real donde el defecto existía, reconstruido con `git archive`. **`ROJO · 1 de 9` — y ese «1 de 9» es la prueba central**: en ese árbol **los otros ocho pasaban** (el chequeo 7 daba **verde** con `2026-08-04 · 101 cierres`) y el 9 lo cazó en **las tres dimensiones a la vez**. Cumple también la regla de S21: el valor de referencia **lo produjo la historia del repo, no el cable**. Evidencia: `docs/audits/s22-completitud-2026-08-09.md`.
- [x] Cada sub-chequeo tiene su **anti-falso-positivo** sembrado y en verde *(verificación: `bateria-sembrada.sh` — gate `na`, sufijo `(N)`, sesión abierta)* → **cumplido, 3 de 3**: un gate `na` legítimo sin `.sh` en disco (typecheck, por `006`) ✓ VERDE · un asiento de `CHANGELOG` con el sufijo `(N)` de `023` ✓ VERDE —el modo de falla que S17 ya pagó una vez con la estampa— · una sesión en las vistas que el plano **no** define ✓ VERDE. **El tercero no es hipotético: S15 está así HOY**, porque nació como hallazgo de corrida y el plano nunca la incorporó. Por eso (c) va en **una sola dirección**: plano → vistas.
- [x] El chequeo 9 **FRENA** (`exit 2`) con motivo si no puede evaluar *(verificación: escenario sembrado sin tracker)* → **cumplido, 3 escenarios**: sin `PLAN.md` · el plano sin encabezados de sesión (enumerar cero **no es un verde**) · `CHANGELOG` con fecha ilegible. Los tres ✓ `exit 2` con su motivo y la frase «Esto NO es un verde». **El tercero es el que cazó el defecto de anclaje**: esperaba FRENA y daba ROJO.
- [x] El **límite del chequeo 9 queda escrito en el script**: qué sigue siendo juicio y no cuenta *(verificación: inspección — el comentario existe y nombra lo que NO cubre)* → **cumplido**: el chequeo declara que mide **COMPLETITUD, no VERACIDAD** — que un gate esté declarado no prueba que su nota diga la verdad, que haya asiento no prueba que describa lo que pasó, que una sesión figure no prueba que su estado sea el real. **Lo que vuelve imposible es el atraso en silencio por OMISIÓN.** Y quedan declaradas las tres cosas que **ningún** cable de este repo puede ver: el artifact publicado, el estado de los delegados y la frescura de `references/` contra su fuente — las tres exigen red o credenciales, o sea perímetro.
